...
app.get("/winding", (req, res) => {
	con.query(
		"SELECT tracking1.orderNo, tracking1.processId, cust_order.company FROM tracking1 INNER JOIN cust_order ON tracking1.orderNo=cust_order.order_no where tracking1.processId=1",
...
app.post("/winding", (req, res) => {
	...
	const yarn_received = params.yarn_received;
	const no_of_cones = params.no_of_cones;
	const size_of_cones = params.size_of_cones;
	const no_of_doffs = params.no_of_doffs;
	const weight_of_cones = params.weight_of_cones;
	const weight_of_waste = params.weight_of_waste;
